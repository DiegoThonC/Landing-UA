...
const LanguageSelect = ({ lang, countryFlag }) => {
  ...
  const [flag, setFlag] = useState(
    countryFlag || localStorage.getItem("countryFlag")
  );
  ...
  return (
    <div className="relative">
      <button
        className="flex pl-28 font-semibold text-2xl uppercase items-center gap-2 lg:text-base lg:text-white lg:pl-0 md:pt-1"
      ...
      >
        ...
        <img
          className="h-4 w-7"
          alt=""
          src={flag ?? localStorage.getItem("countryFlag")}
        />
      </button>
      {collapse && (
        <ul className="text-base uppercase absolute left-28 lg:-left-10 md:top-8 px-5 py-2 text-center md:text-white border border-solid border-gray-800 bg-black z-10">
          <li
            className="hover:text-orange-400 cursor-pointer text-white flex gap-3 items-center"
            onClick={() => changeLang("ua", ukraine)}
          >
